...
def train(model,
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(int((T - lookback) / batchsize) + 1):  # 循环处理数据块
            ...
            std = torch.std(rets_train+ 1e-8) +1e-8 #标准差
        ...
        if log_dev_progress and epoch % log_dev_progress_freq == 0:
            ...
            full_std = np.std(rets_full+1e-8)
